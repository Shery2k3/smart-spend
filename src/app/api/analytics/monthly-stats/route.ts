...
export async function GET() {
  try {
    ...
    // Debt stats: money owed TO the user (given) vs money the user OWES (taken)
    const amountOwedToYou = debts
      .filter((d) => d.debtType === "given")
      .reduce((sum, d) => sum + d.amountRemaining, 0);

    const amountYouOwe = debts
      .filter((d) => d.debtType === "taken")
      .reduce((sum, d) => sum + d.amountRemaining, 0);

    const netDebtPosition = amountOwedToYou - amountYouOwe;
